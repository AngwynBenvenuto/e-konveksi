    ...
    <script>
        $(document).ready(function() {
            var table = $("#role_table").DataTable({
                ajax: "{{ route('admin.setting.role.show') }}",
                columns: [
                    { "data": "name" },
                    { "data": "description" },
                    { "data": "created_at" },
            ...
            $(".btnAdd").click(function() {
                window.location.href = "{{ route('admin.setting.role.create') }}";
            });


            $("#role_table").on('click', '.btnEdit', function () {
                var id = $(this).data('id');
                var url = '{{ route("admin.setting.role.edit", ":id") }}';

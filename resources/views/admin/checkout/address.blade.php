...
@section('content')
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="card mb-4">
                            <div class="card-body">
                                <div class="form-group">
                                    <label class="form-label">Pilih Data Penjahit: </label>
                                    <?php
                                        $penjahits = \App\Models\Penjahit::where('status', '>', 0)->get();
                                        if($penjahits != null) :
                                            foreach($penjahits as $penjahit) :
                                    ?>
                                        <div class="form-controls-stacked" style="position:relative;margin-top:10px;">
                                            <label class="label-block">
                                                <input name="penjahit" type="radio" class="penjahit penjahit-{{$penjahit->id}}" value="{{ $penjahit->id }}">&nbsp;{{ $penjahit->name }}
                                            </label>
                                        </div>
                                    <?php
                                            endforeach;
    ...
    <script>
        $(document).ready(function() {
            ...
            //
            $(".penjahit").each(function() {
                $(this).click(function() {
                    var penjahit_id = this.value;
                    $.ajax({
                        url: "{{ route('api.penjahit') }}",
                        data: { penjahit_id: penjahit_id },
                        dataType: 'json',
                        method: "POST",
                        success: function(response) {
                            if(response.errCode == 0) {
                                var dataPenjahit = response.data;
                                $("#billing_name").val(dataPenjahit.name);
                                $("#billing_email").val(dataPenjahit.email);
                                $("#billing_address").html(dataPenjahit.address);
                                $("#billing_phone").val(dataPenjahit.phone);
                                $("#billing_province_id").val(dataPenjahit.province_id).trigger('change');
                                $("#billing_city_id").val(dataPenjahit.city_id).trigger('change');
                                $("#billing_zipcode").val(dataPenjahit.postal_code);
                            }
                        }
                    })
                });
